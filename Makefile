...
CFLAGS  := -DRSGD_MAJOR=$(MAJOR) -DRSGD_MINOR=$(MINOR) -DRSGD_REVISION=$(REVISION) -Wall -Wextra -Wno-unused-result -lstdc++fs
LDFLAGS := -fPIC
AR      := ar
ARFLAGS := rcs
...
main: $(DEP_DIR) $(OBJ_DIR) $(OUT_DIR) $(OBJS)    ##@examples Compile main example.
  ...
  @$(CXX) $(OBJS) $(EXAMPLE)/main.cpp -o $(OUT_DIR)/rsgd $(CFLAGS) $(LDFLAGS)
  @printf "[done]\n"
...
HELP_FUN = \
  ...
  print "\t\t\t$(LBLUE)rSGD Makefile$(RESET)\n"; \
