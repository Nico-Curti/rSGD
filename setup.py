...
REQUIRES_PYTHON = '>=2.7'
VERSION = None
KEYWORDS = "gradient-descent deep-neural-networks spin-glass"
...
if not os.path.isfile(os.path.join(here, 'lib', 'librsgd.so')):
...
about = {}
if not VERSION:
  with open(os.path.join(here, 'ReplicatedStochasticGradientDescent', '__version__.py')) as f:
    exec(f.read(), about)
else:
...
if 'GCC' in cpp_compiler or 'CLANG' in cpp_compiler:
  ...
  cpp_compiler_args = []
extra_compile_args = cpp_compiler_args + ['-DNDEBUG', '-DVERBOSE', '-DNPY_NO_DEPRECATED_API=NPY_1_7_API_VERSION']
...
setup(
  ...
  download_url        = URL,
  keywords            = KEYWORDS,
  packages            = find_packages(include=['ReplicatedStochasticGradientDescent', 'ReplicatedStochasticGradientDescent.*'], exclude=('test',)),
